...
TEXT = {
    ...
    "num": {"cn": "数值设置", "en": "Numerics"},
    "num_tip": {"cn": "建议：401~1201 比较稳。更高 N 会更慢，但曲线更平滑。", "en": "Suggestion: 401–1201 is robust. Larger N is slower but smoother."},
    # Plots
    ...
    "explain_hint": {"cn": "🔎 结果解读小提示（可选）", "en": "🔎 Interpretation hints (optional)"},
    # Report
    ...
    "center": {"cn": "高温超导联合工程中心@广东东莞松山湖", "en": "HTS Joint Engineering Center @ Songshan Lake, Dongguan, Guangdong"},
}
...
# =========================================================
# 9) Sidebar：语言 + 参数输入（无预设、无清空结果；保留清空历史）
# =========================================================
with st.sidebar:
    ...
    with st.expander(t("bc"), expanded=True):
        T_H = st.number_input("T_H (K)", value=100.0, step=1.0, help="Hot boundary temperature / 热端边界温度")
        T_C = st.number_input("T_C (K)", value=7.0, step=1.0, help="Cold boundary temperature / 冷端边界温度")
        I = st.number_input("I (A)", value=240.0, step=10.0, help="Lead current / 引线电流")

    with st.expander(t("cu"), expanded=True):
        A_cu_mm2 = st.number_input("A_cu (mm²)", value=50.0, step=1.0)
    ...
    with st.expander(t("ybco"), expanded=True):
        ...
        A_shunt_mm2 = st.number_input("A_shunt (mm²)", value=20.8, step=0.1, format="%.2f")

    with st.expander(t("joint"), expanded=True):
        R_joint_uohm = st.number_input("R_joint (μΩ)", value=39.11, step=0.1, format="%.3f")
...
# ---- YBCO joint temperature check ----
tj = out.get("T_joint", None) if isinstance(out, dict) else None
